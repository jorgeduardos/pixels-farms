...
var sortable = new sortablejs__WEBPACK_IMPORTED_MODULE_0__["default"](farmListDom, {
  animation: 150,
  ghostClass: 'ghost',
...
function addFarm(form) {
  var crop = crops[form.type];
  var match = regex.exec(form.farmNumber);
  var number = match == null ? form.farmNumber : match[0]; //regex url
...
farmListDom.addEventListener('click', function (e) {
  ...
  if (e.target.classList.contains('start-farm')) {
    ...
    if (dev) {
      ...
      console.log('farm node to be deleted: ', _farmDom);
...
function continueFarmTimer(farm) {
  ...
  var timerDom = farmDom.querySelector('.timer');
  var now = Date();
  var farmTimer = timerCalculate(now, farm.startTime, farm.crop.sproutTime); //saving timer pointer in farm

  if ((Math.sign(farmTimer.hours) == 0 || Math.sign(farmTimer.hours) == -1) && (Math.sign(farmTimer.minutes) == 0 || Math.sign(farmTimer.minutes) == -1) && (Math.sign(farmTimer.seconds) == 0 || Math.sign(farmTimer.seconds) == -1)) {
    farmDom.classList.add('completed');
  ...
  farm.timer = timer;
  farmDom.querySelector('.start-farm').disabled = true; // timer.start({ countdown: true, startValues: {seconds: 10} });

  timer.start({
    countdown: true,
    startValues: farmTimer
  });
...
function timerCalculate(fDate, oDate, sproutTimer) {
  ...
  var delta = Math.abs(futureDate - oldDate) / 1000; // calculate (and subtract) whole hours
  ...
  var sproutDate = new Date(1994, 4, 20, sproutTimer.hours, sproutTimer.minutes, sproutTimer.seconds, 0); //create date with time that has passes since start
